...
if 'show_salvage_form' not in st.session_state:
    st.session_[state.show](http://state.show)_salvage_form = False
...
if 'crown_density' not in st.session_state:
    st.session_[state.crown](http://state.crown)_density = default_values["crown_density"]
...
def calculate_avi_and_volumes(crown_density, avg_stand_height, dom_species, dom_cover, sec_species, sec_cover, area, region):
...
with col_nav2:
    if st.button("Save Entry"):
        ...
        calculate_avi_and_volumes(
            st.session_[state.crown](http://state.crown)_density,
        ...
        if st.session_state.current_entry_index >= 0 and st.session_state.edit_mode and st.session_state.results_log:
            ...
            entry_data = {
                ...
                "crown_density": st.session_[state.crown](http://state.crown)_density,
...
with col1:
    ...
    if (st.session_state.edit_mode and
        ...
        st.session_[state.crown](http://state.crown)_density = entry.get("crown_density", default_values["crown_density"])
...
calculate_avi_and_volumes(crown_density, avg_stand_height, dom_species, dom_cover, sec_species, sec_cover, area, region)
...
with col2:
    ...
    def convert_lsd_to_p3(lsd):
        ...
        if match:
            meridian = [match.group](http://match.group)(1) # Last group is the meridian number
    ...
    if species_sel:
        ...
        current_date = [datetime.date](http://datetime.date)(2025, 10, 1)
        p3_date = [datetime.date](http://datetime.date)(p3_year, p3_month, 1)
...
if st.button("Finish (Fill Salvage Draft)", key="finish_salvage"):
    st.session_[state.show](http://state.show)_salvage_form = True
# --- Salvage form & Word export ---
if st.session_[state.show](http://state.show)_salvage_form:
    ...
    if st.button("Add Another Disposition"):
        ...
        st.rerun()
    salvage_waiver = [st.radio](http://st.radio)(
    ...
    def fill_template():
        ...
        [run.font.name](http://run.font.name) = "Times New Roman"; run.font.size = Pt(11)
        run.font.bold = True; run.font.underline = True
        p.paragraph_[format.space](http://format.space)_after = Pt(0)
        # Disposition
        p = doc.add_paragraph(); p.alignment = 1
        r1 = p.add_run("Disposition: "); [r1.font.name](http://r1.font.name) = "Times New Roman"; r1.font.size = Pt(10); r1.font.bold = True
        r2 = p.add_run(disposition); [r2.font.name](http://r2.font.name) = "Times New Roman"; r2.font.size = Pt(10); r2.font.bold = False
        p.paragraph_[format.space](http://format.space)_after = Pt(0)
        # Legal Land Location
        p = doc.add_paragraph(); p.alignment = 1
        r1 = p.add_run("Legal Land Location: "); [r1.font.name](http://r1.font.name) = "Times New Roman"; r1.font.size = Pt(10); r1.font.bold = True
        r2 = p.add_run(legal_loc); [r2.font.name](http://r2.font.name) = "Times New Roman"; r2.font.size = Pt(10); r2.font.bold = False
        p.paragraph_[format.space](http://format.space)_after = Pt(0)
        # Horizontal line
        p = doc.add_paragraph()
        ...
        [run.font.name](http://run.font.name) = "Times New Roman"; run.font.size = Pt(12); run.font.bold = True
        p.paragraph_[format.space](http://format.space)_before = Pt(0); p.paragraph_[format.space](http://format.space)_after = Pt(6)
        # Vegetation subheader
        p = doc.add_paragraph()
        run = p.add_run("Vegetation (check all that apply)")
        [run.font.name](http://run.font.name) = "Times New Roman"; run.font.size = Pt(10); run.font.bold = True
        p.paragraph_[format.space](http://format.space)_before = Pt(0); p.paragraph_[format.space](http://format.space)_after = Pt(0)
        ...
        for left, right in rows:
            p = doc.add_paragraph(); p.paragraph_[format.space](http://format.space)_before = Pt(0); p.paragraph_[format.space](http://format.space)_after = Pt(0)
            ...
            if right == "Treed wetland":
                ...
                [run.font.name](http://run.font.name) = "Times New Roman"; run.font.size = Pt(10); run.font.bold = True; run.font.underline = True
                ...
                if "Other (specify)" in vegetation and other_specify_details:
                    ...
                    [run.font.name](http://run.font.name) = "Times New Roman"; run.font.size = Pt(10); run.font.bold = False; run.font.underline = True
                run = p.add_run("\t\t")
                [run.font.name](http://run.font.name) = "Times New Roman"; run.font.size = Pt(10); run.font.bold = True
        ...
        p = doc.add_paragraph(); p.paragraph_[format.space](http://format.space)_before = Pt(6); p.paragraph_[format.space](http://format.space)_after = Pt(0)
        ...
        [run1.font.name](http://run1.font.name) = "Times New Roman"; run1.font.size = Pt(10); run1.font.bold = True
        run2 = p.add_run(f"{total_c_vol:.1f}");
        [run2.font.name](http://run2.font.name) = "Times New Roman"; run2.font.size = Pt(10); run2.font.bold = False; run2.font.underline = True
        run3 = p.add_run(f" m³");
        [run3.font.name](http://run3.font.name) = "Times New Roman"; run3.font.size = Pt(10); run3.font.bold = False; run3.font.underline = True
        run4 = p.add_run(f" or ");
        [run4.font.name](http://run4.font.name) = "Times New Roman"; run4.font.size = Pt(10); run4.font.bold = True
        run5 = p.add_run(f"{total_c_load:.1f}");
        [run5.font.name](http://run5.font.name) = "Times New Roman"; run5.font.size = Pt(10); run5.font.bold = False; run5.font.underline = True
        run6 = p.add_run(f" loads");
        [run6.font.name](http://run6.font.name) = "Times New Roman"; run6.font.size = Pt(10); run6.font.bold = False; run6.font.underline = True
        ...
        [run3.font.name](http://run3.font.name) = "Times New Roman"; run3.font.size = Pt(10); run3.font.bold = True
        run4 = p.add_run(f"{pine_pct}%");
        [run4.font.name](http://run4.font.name) = "Times New Roman"; run4.font.size = Pt(10); run4.font.bold = False; run4.font.underline = True
        run5 = p.add_run(f" Other ");
        [run5.font.name](http://run5.font.name) = "Times New Roman"; run5.font.size = Pt(10); run5.font.bold = True
        ...
        [doc.save](http://doc.save)[](http://tmp.name)
        return [tmp.name](http://tmp.name), filename
    if st.button(
        ...
        if out_path:
            ...
            with open(out_path, "rb") as f:
                b64 = base64.b64encode([f.read](http://f.read)()).decode()
                st.markdown(
...
if uploaded_files:
    for uploaded_file in uploaded_files:
        ...
        zip_path = temp_base_dir / uploaded_[file.name](http://file.name)
        with open(zip_path, "wb") as f:
            f.write(uploaded_file.getbuffer())
       
        with open(log_file, "a") as log:
            log.write(f"\nProcessing {zip_[path.name](http://path.name)}...\n")
            st.sidebar.write(f"Processing {zip_[path.name](http://path.name)}...")
            ...
            try:
                ...
                log.write(f"Extracted {zip_[path.name](http://path.name)} to {temp_dir}\n")
                # Find shapefiles
                shapefiles = list(temp_dir.glob("*.shp"))
                if not shapefiles:
                    log.write(f"No shapefiles found in {zip_[path.name](http://path.name)}, skipping.\n")
                    st.sidebar.warning(f"No shapefiles found in {zip_[path.name](http://path.name)}, skipping.")
                    continue
                if len(shapefiles) > 1:
                    log.write(f"Warning: Multiple shapefiles found in {zip_[path.name](http://path.name)}. Processing only: {shapefiles[0]}\n")
                    st.sidebar.warning(f"Warning: Multiple shapefiles found. Using: {shapefiles[0]}")
                # Load the shapefile
                try:
                    gdf = [gpd.read](http://gpd.read)_file(shapefiles[0])
                ...
                if [gdf.geometry.is](http://gdf.geometry.is)_valid.all():
                ...
                try:
                    dissolved_[gdf.to](http://gdf.to)_file(out_file)
    ...
    output_zip_path = temp_base_dir / "dissolved_[shapefiles.zip](http://shapefiles.zip)"
    with zipfile.ZipFile(output_zip_path, "w", [zipfile.ZIP](http://zipfile.ZIP)_DEFLATED) as zipf:
        for root, _, files in os.walk(output_dir):
            for file in files:
                zipf.write(os.path.join(root, file), os.path.relpath(os.path.join(root, file), output_dir))
    # Provide download link for the zipped outputs
    with open(output_zip_path, "rb") as f:
        [st.sidebar.download](http://st.sidebar.download)_button(
            label="Download All Dissolved Shapefiles (Zip)",
            data=f,
            file_name="dissolved_[shapefiles.zip](http://shapefiles.zip)",
    ...
    with open(log_file, "rb") as f:
        [st.sidebar.download](http://st.sidebar.download)_button(
            ...
            data=f,
            file_name=log_[file.name](http://file.name),
            mime="text/plain"
        )
